...
def calculate_demographic_data(print_data=True):
    ...
    country_earnings = df.groupby('native-country')
    higher_earners = df[df['salary'] == '>50K']
    highest_earning_country_percentage = 0
    highest_earning_country = ''
    for country, group in country_earnings:
        total = group.shape[0]
        rich = higher_earners[higher_earners['native-country'] == country].shape[0]
        percent = rich / total * 100 if total > 0 else 0
        if percent > highest_earning_country_percentage:
